...
function toggleChecklistRowFromTarget(target){
  try{
    const row = target?.closest?.('.notes-checklist-row');
    if(!row) return false;
...
function wireNotesEditor(){
  ...
  const rowPointerHandler = (e)=>{
    const isCheckboxTap = !!e.target?.closest?.('.notes-checklist-box');
    ...
    if(isCheckboxTap){
      ...
      e.preventDefault();
      e.stopPropagation();
      toggleChecklistRowFromTarget(e.target);
      return;
    }

    if(row && text && !e.target?.closest?.('.notes-checklist-text')){
      e.preventDefault();
      focusChecklistTextEnd(text);
    }
  };
  ...
  ed.addEventListener("click", (e)=>{
    if(e.target?.closest?.('.notes-checklist-box')){
      e.preventDefault();
      e.stopPropagation();
      return;
    }
    if(e.target?.classList?.contains('notes-checklist-text') || e.target?.classList?.contains('notes-plain-line')){
      activeNotesEditable = e.target;
    }
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
  });
