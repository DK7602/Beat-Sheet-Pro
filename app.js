...
const els = {
  ...
  pageHeaderDock: need("pageHeaderDock"),
...
function bindPageHeaderDockTitleInput(p){
  try{
    const dock = els.pageHeaderDock;
    if(!dock || dock.dataset.boundInput === "1") return;
    dock.dataset.boundInput = "1";
    dock.addEventListener("input", (e)=>{
      const inp = e.target?.closest?.('input[data-sec-title]');
      if(!inp) return;
      const key = inp.getAttribute("data-sec-title");
      if(!key) return;
      const sec = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:true });
      sec.title = inp.value;
      document.querySelectorAll(`input.sectionPill[data-sec-title="${key}"], input.pageTitlePill[data-sec-title="${key}"]`).forEach(other=>{
...
function updatePageHeaderDock(p){
  try{
    const dock = els.pageHeaderDock;
    if(!dock) return;

    const key = getVisibleRealPageKeyFromPager(p) || p.activeSection || "full";
    const page = getActiveRealPageEl(key);
    const titleRow = page?.querySelector?.(".pageTitleRow");
    if(!titleRow){
      dock.innerHTML = "";
      dock.dataset.pageKey = "";
      return;
    }

    const sameKey = dock.dataset.pageKey === key;
    const existingInput = sameKey ? dock.querySelector('input[data-sec-title]') : null;
    const existingValue = existingInput ? existingInput.value : null;
    const shouldRestoreFocus = sameKey && existingInput && document.activeElement === existingInput;

    dock.innerHTML = "";
    const clone = titleRow.cloneNode(true);
    clone.style.display = "flex";
    dock.appendChild(clone);
    dock.dataset.pageKey = key;
    dock.dataset.boundInput = "0";

    const dockInput = dock.querySelector('input[data-sec-title]');
    if(dockInput && sameKey && existingValue != null){
      dockInput.value = existingValue;
      if(shouldRestoreFocus){
        try{
          const pos = existingInput.selectionStart ?? existingValue.length;
          dockInput.focus();
          dockInput.setSelectionRange(pos, pos);
        }catch(_e){}
      }
    }

    bindPageHeaderDockTitleInput(p);
  }catch(_e){}
}
...
function renderStickyPageHeader(pageKeyOverride=null){
  const p = getActiveProject();
  const dock = els.pageHeaderDock || document.getElementById("pageHeaderDock");
  if(!p || !dock) return;

  const key = pageKeyOverride || p.activeSection || "full";
  dock.innerHTML = "";
  dock.setAttribute("data-page-key", key);

  const row = document.createElement("div");
  row.className = "pageTitleRow stickyTop";

  let titleEl;
  if(key === "full"){
    titleEl = document.createElement("div");
    titleEl.className = "pageTitle";
    titleEl.textContent = "Full Song View";
  }else{
    const sec = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:true });
    titleEl = document.createElement("input");
    titleEl.type = "text";
    titleEl.spellcheck = false;
    titleEl.className = "pageTitlePill sectionPill";
    titleEl.placeholder = "Song Part";
    titleEl.value = (sec.title || "").trim();
    titleEl.setAttribute("data-sec-title", key);
    titleEl.addEventListener("input", ()=>{
      sec.title = titleEl.value;
      touchProject(p);
      document.querySelectorAll(`input.sectionPill[data-sec-title="${key}"]`).forEach(inp=>{
        if(inp !== titleEl) inp.value = titleEl.value;
      });
    });
  }
  ...
  addBtn.addEventListener("click", (e)=>{
    e.preventDefault(); e.stopPropagation();
    addNextPage(p, key, key === "full" ? { stayOnFull:true } : undefined);
  });
...
function getRealPageKeyFromCarouselIdx(p, idx){
  const order = getActivePageOrder(p);
  const CAROUSEL_ORDER = [order[order.length - 1], ...order, order[0]];
...
function setActiveSectionFromIdx(p, idx){
  const key = getRealPageKeyFromCarouselIdx(p, idx);
...
function setupCarouselPager(pagerEl, p){
  ...
  const snapToActive = () => {
    ...
    renderStickyPageHeader(p.activeSection || "full");
  };

  window.addEventListener("resize", snapToActive);

  let settleTimer = null;
  const settlePagerToIdx = (rawIdx) => {
    const order = getActivePageOrder(p);
    ...
    if(idx === 0){
      idx = carousel.length - 2;
      snapToIdx(pagerEl, idx, "auto");
    }else if(idx === carousel.length - 1){
      idx = 1;
      snapToIdx(pagerEl, idx, "auto");
    }
  ...
  pagerEl.addEventListener("scroll", ()=>{
    ...
    settleTimer = setTimeout(()=>{
      settlePagerToIdx(getCurrentIdx(pagerEl));
    }, 140);
  }, { passive:true });
...
const mountEl = pageEl ? pageEl.querySelector(".sectionMount") : null;
  ...
  p.activeSection = wantKey;
  renderStickyPageHeader();
  requestAnimationFrame(renderStickyPageHeader);
