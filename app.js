...
const els = {
  ...
  notesCheckboxBtn: need("notesCheckboxBtn"),
...
function notesCleanText(s){
  return String(s || "").replace(/[\u200B\u2060\ufeff]/g, "").replace(/\xa0/g, " ");
}
function getNotesCheckboxLine(node){
  try{
    const el = (node && node.nodeType === Node.ELEMENT_NODE) ? node : node?.parentElement;
    return el?.closest?.(".notesCheckLine") || null;
  }catch(_e){
    return null;
  }
}
function getNotesCheckboxTextEl(line){
  return line?.querySelector?.(".notesCheckText") || null;
}
function getNotesCheckboxBoxEl(line){
  return line?.querySelector?.(".notesCheckBox") || null;
}
function isNotesLineEmpty(line){
  const textEl = getNotesCheckboxTextEl(line);
  const html = normalizeNotesHtml(textEl?.innerHTML || "");
  const text = notesCleanText(textEl?.textContent || "").trim();
  return !html && !text;
}
function createNotesPlainLine(html=""){
  const line = document.createElement("div");
  line.className = "notesPlainLine";
  line.innerHTML = html || "<br>";
  return line;
}
function createNotesCheckboxLine(html="", checked=false){
  const line = document.createElement("div");
  line.className = "notesCheckLine" + (checked ? " checked" : "");
  ...
  text.innerHTML = html || "<br>";

  line.appendChild(box);
  line.appendChild(text);
  return line;
}
function setNotesCheckboxChecked(line, checked){
  if(!line) return;
  line.classList.toggle("checked", !!checked);
  const box = getNotesCheckboxBoxEl(line);
  if(box){
    box.textContent = checked ? "☑" : "☐";
    box.setAttribute("aria-checked", checked ? "true" : "false");
  }
...
function focusNotesNodeEnd(node){
  try{
    const target = node?.nodeType ? node : null;
    if(!target) return;
...
function ensureNotesEditorHasFallbackLine(){
  const ed = els.notesEditor;
  if(!ed) return;
  const hasReal = Array.from(ed.childNodes || []).some((n)=>{
    if(n.nodeType === Node.TEXT_NODE) return !!notesCleanText(n.nodeValue || "").trim();
...
function insertCheckboxAtNotesSelection(){
  const ed = els.notesEditor;
  ...
  const existingLine = getNotesCheckboxLine(range.startContainer);
  if(existingLine){
    const textEl = getNotesCheckboxTextEl(existingLine);
    if(textEl) focusNotesNodeStart(textEl);
    saveNotesSelection();
    return;
  }

  const block = (range.startContainer?.nodeType === Node.ELEMENT_NODE ? range.startContainer : range.startContainer?.parentElement)?.closest?.("div,p,li");
  const boxLine = createNotesCheckboxLine();

  if(block && ed.contains(block)){
    const html = normalizeNotesHtml(block.innerHTML);
    if(html) getNotesCheckboxTextEl(boxLine).innerHTML = html;
    block.replaceWith(boxLine);
  }else{
    range.collapse(true);
    range.insertNode(boxLine);
  }

  const textEl = getNotesCheckboxTextEl(boxLine);
  if(textEl) focusNotesNodeStart(textEl);
  saveNotesFromEditor();
...
function focusNotesAfterDeletingCheckbox(line){
  try{
    const ed = els.notesEditor;
    const prev = line?.previousSibling || null;
    const next = line?.nextSibling || null;
    line?.remove?.();
    ensureNotesEditorHasFallbackLine();

    const target = prev || next || ed?.firstChild || ed;
    if(!target) return;

    const textTarget = getNotesCheckboxTextEl(target) || target;
    focusNotesNodeEnd(textTarget);
  }catch(_e){}
}
...
function wireNotesEditor(){
  ...
  ed.addEventListener("click", (e)=>{
    ...
    if(!box || !ed.contains(box)) return;
    e.preventDefault();
    const line = getNotesCheckboxLine(box);
    if(!line) return;
    const checked = !line.classList.contains("checked");
    setNotesCheckboxChecked(line, checked);
    const textEl = getNotesCheckboxTextEl(line);
    if(textEl) focusNotesNodeEnd(textEl);
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
  });

  ed.addEventListener("keydown", (e)=>{
    const line = getNotesCheckboxLine(e.target) || getNotesCheckboxLine(window.getSelection?.()?.anchorNode);
    if(!line) return;
    ...
    if(e.key === "Backspace"){
      ...
      const range = sel.getRangeAt(0);
      if(!range.collapsed) return;
      if(!isNotesLineEmpty(line)) return;
      if(!isRangeAtStartOfNode(range, textEl)) return;

      e.preventDefault();
      focusNotesAfterDeletingCheckbox(line);
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
    }
  });
...
els.notesCheckboxBtn?.addEventListener("click", ()=> insertCheckboxAtNotesSelection());
