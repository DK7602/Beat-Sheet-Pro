...
let lastStableHeaderH = 0;
function headerKeyboardLockActive(){
  try{
    ...
    const typing = !!(ae && (
      ...
      ae.tagName === "INPUT" ||
      ae === els?.notesEditor ||
      (ae.isContentEditable && ae.closest && ae.closest("#notesEditor"))
    ));
    const vv = window.visualViewport;
    const keyboardOpen = !!(vv && ((window.innerHeight - vv.height - vv.offsetTop) > 120));
    return typing && keyboardOpen;
  }catch(_e){
    return false;
  }
}
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      ...
      const freezeDuringTyping = headerKeyboardLockActive() && lastStableHeaderH > 0;

      if(freezeDuringTyping){
        h = lastStableHeaderH;
      } else if(sticky){
        h = Math.ceil((sticky.offsetTop || 0) + (sticky.offsetHeight || sticky.getBoundingClientRect().height || 0));
      } else {
        h = Math.ceil(header.offsetHeight || header.getBoundingClientRect().height || 0);
      }

      if(h > 0 && !freezeDuringTyping) lastStableHeaderH = h;
