...
const NOTES_REFRESH_REOPEN_KEY = `${KEY_PREFIX}notes_reopen_after_refresh_v1`;
...
function triggerAppRefresh(){
  try{
    if(notesOpen){
      saveNotesFromEditor();
      sessionStorage.setItem(NOTES_REFRESH_REOPEN_KEY, "1");
    }else{
      sessionStorage.removeItem(NOTES_REFRESH_REOPEN_KEY);
    }
  }catch(_e){}
...
(async function boot(){
  ...
  renderAll();
  syncHeaderHeightVar();
  attachPullToRefresh(els.barsScroller, ()=>triggerAppRefresh());
  ...
  syncHeaderHeightVar();
  try{
    if(sessionStorage.getItem(NOTES_REFRESH_REOPEN_KEY) === "1") {
      sessionStorage.removeItem(NOTES_REFRESH_REOPEN_KEY);
      openNotes();
    }
  }catch(_e){}
