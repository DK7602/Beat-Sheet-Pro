...
function buildRhymeBuckets(seed, perfectData, nearData, soundLikeData){
  ...
  return {
    perfect: perfect.slice(0, 18).map(x=>x.word),
    near: near.slice(0, 18).map(x=>x.word)
  };
}
...
async function updateRhymes(seed){
  ...
  try{
    ...
    const [perfectRes, nearRes, soundLikeRes] = await Promise.all([
      fetch(`https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&md=f&max=80`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=f&max=80`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?sl=${encodeURIComponent(w)}&md=f&max=60`, { signal: rhymeAbort.signal })
    ]);
...
function splitRhymeRows(words){
  const list = Array.isArray(words) ? words.filter(Boolean) : [];
  if(!list.length) return [[], []];
  const row1 = [];
  const row2 = [];
  list.forEach((word, i)=>{
    (i % 2 === 0 ? row1 : row2).push(word);
  });
  return [row1, row2];
}
function renderRhymeRow(words){
  if(!words || !words.length) return "";
  return `<div class="rhymeGroupChips">${words.map(w=>`<button type="button" class="rhymeChip" data-rhyme="${escapeHtml(w)}">${escapeHtml(w)}</button>`).join("")}</div>`;
}
function renderRhymes(groups){
  ...
  const renderGroup = (label, words) => {
    if(!words || !words.length) return "";
    const [row1, row2] = splitRhymeRows(words);
    return `
      <div class="rhymeGroup">
        <div class="small rhymeGroupLabel">${escapeHtml(label)}</div>
        <div class="rhymeGroupRows">
          ${renderRhymeRow(row1)}
          ${renderRhymeRow(row2)}
        </div>
      </div>
