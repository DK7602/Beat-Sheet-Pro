...
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      ...
      // Use the natural content height as the source of truth.
      // This keeps the full top window stable and prevents it from collapsing
      // when viewport/UI chrome shifts during scroll or recording.
      const naturalContentH = Math.ceil((hs?.scrollHeight || header.scrollHeight || headerRect.height || 0));

      // Keep a floor at least through the drum row so the sticky controls never get clipped.
      let stickyBottomH = 0;
      if(sticky){
        const stickyOffsetTop = Number.isFinite(sticky.offsetTop) ? sticky.offsetTop : 0;
        stickyBottomH = Math.ceil(stickyOffsetTop + sticky.offsetHeight + padTop + padBottom);
      }

      h = Math.max(naturalContentH, stickyBottomH, 140);
      ...
      h = Math.max(0, Math.ceil(header.getBoundingClientRect().height || 0));
      header.style.height = "";
...
window.addEventListener("orientationchange", scheduleHeaderHeightSync);
...
async function refreshMicProcessingProfile(){
  ...
  if(micGain){
    // Slight trim while drums/backing are active to reduce any remaining speaker bleed.
    micGain.gain.value = wantsEchoProtection ? 0.13 : 0.15;
  }
}
...
async function ensureMic(){
  ...
  await refreshMicProcessingProfile();

  if(micSource && micGain) return;
...
const canStartFromEventTarget = (tgt)=>{
  ...
  // Main-view PTR should only start from the actual bottom window, not from header controls.
  if(refreshProfile !== NOTES_PTR){
    const inMainWindow = (tgt === scrollEl) || !!tgt?.closest?.('#bars');
