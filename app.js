...
function notesBlankTypingHtml(color=getNotesTypingColor()){
  ...
  return `<span style="color:${c};" data-notes-typing-color="${escapeNotesAttr(c)}">​</span>`;
}
...
function seedNotesTypingTarget(node, color=getNotesTypingColor(), placeCaret=true){
  try{
    ...
    const marker = node.querySelector('[data-notes-typing-color]')?.firstChild;
...
function cleanupNotesTypingMarkers(root=els?.notesEditor){
  try{
    if(!root?.querySelectorAll) return;
    root.querySelectorAll('span[data-notes-typing-color]').forEach((span)=>{
      const rawText = String(span.textContent || '').replace(/​/g, '').trim();
      const hasRealChild = Array.from(span.childNodes || []).some((n)=>{
        if(n.nodeType === Node.TEXT_NODE) return String(n.nodeValue || '').replace(/​/g, '').trim().length > 0;
        return !(n.nodeType === 1 && /^(br)$/i.test(n.nodeName || ''));
      });
      if(!rawText && !hasRealChild){
        if(span.parentNode) span.parentNode.removeChild(span);
        return;
      }
      span.removeAttribute('data-notes-typing-color');
    });
  }catch(_e){}
}
...
function noteExec(cmd, val=null){
  ...
  if(cmd === "foreColor"){
    const color = val || "#111111";
    let isCollapsed = true;
    try{
      const sel = window.getSelection?.();
      isCollapsed = !sel || !sel.rangeCount || sel.getRangeAt(0).collapsed;
    }catch(_e){}

    if(isCollapsed){
      // For collapsed carets, seed a live typing marker and keep it in place.
      // Do NOT immediately clean it up here or the browser falls back to the
      // previous inline color state, which causes the "stuck on last color"
      // behavior the user is seeing.
      primeNotesTypingColor(color);
    }else{
      try{ document.execCommand(cmd, false, color); }catch(_e){}
      // Selected text is already colored; any stale typing markers can go.
      cleanupNotesTypingMarkers(ed);
    }
