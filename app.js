...
async function cropNotesPhotoItem(item){
  const imgEl = getNotesPhotoImage(item);
  const src = String(imgEl?.src || '');
  if(!src) return;
  const answer = window.prompt('Crop photo: enter left, top, right, bottom percentages. Example: 10,0,10,0', '0,0,0,0');
  if(answer == null) return;
  const parts = String(answer).split(',').map(v=>parseFloat(v.trim()));
  if(parts.length !== 4 || parts.some(v=>!Number.isFinite(v) || v < 0 || v >= 100)){
    showToast('Crop format: 10,0,10,0');
    return;
  }
  const [leftPct, topPct, rightPct, bottomPct] = parts;
  if((leftPct + rightPct) >= 100 || (topPct + bottomPct) >= 100){
  ...
  try{
    ...
    const sx = Math.round(sw * leftPct / 100);
    const sy = Math.round(sh * topPct / 100);
    const cw = Math.max(1, Math.round(sw * (100 - leftPct - rightPct) / 100));
    const ch = Math.max(1, Math.round(sh * (100 - topPct - bottomPct) / 100));
    const canvas = document.createElement('canvas');
    canvas.width = cw;
    canvas.height = ch;
    const ctx = canvas.getContext('2d');
