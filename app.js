...
function normalizeNotesHtml(html){
  ...
  try{
    ...
    const text = String(tmp.textContent || "")
      ...
      .replace(/\u00A0/g, " ")
      .trim();
    const structural = String(tmp.innerHTML || "")
      .replace(/\u200B/g, "")
      .replace(/&nbsp;/gi, " ")
      .replace(/<br\s*\/?\s*>/gi, "")
      .replace(/<\/?(div|p|span|font|b|strong|i|em|u|s|strike)\b[^>]*>/gi, "")
      .trim();
...
function wireNotesEditor(){
  ...
  ed.addEventListener("beforeinput", (e)=>{
    try{
      ...
      if(/^insert/i.test(type) && !normalizeNotesHtml(ed.innerHTML)){
        applyNotesTypingColor(getNotesTypingColor(), false);
      }
    }catch(_e){}
  });
  ...
  ed.addEventListener("input", ()=>{
    const normalized = normalizeNotesHtml(ed.innerHTML);
    if(!normalized){
      ed.innerHTML = "";
      applyNotesTypingColor(getNotesTypingColor(), false);
      try{
