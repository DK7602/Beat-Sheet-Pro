...
function setAutoScroll(v){
  ...
  const p0 = (typeof getActiveProject === "function") ? getActiveProject() : null;
  const visibleKeyBeforeToggle = turningOn ? (getVisibleRealPageKeyFromPager(p0) || p0?.activeSection || null) : null;
  const pendingFullEditorAnchor = (turningOn && visibleKeyBeforeToggle === "full") ? getFirstVisibleFullEditorTarget() : null;
  autoScrollOn = !!v;
  if(!autoScrollOn){
    autoScrollPaused = false;
    autoScrollPauseSnapshot = null;
  ...
  if(turningOn){
    ...
    if(p){
      const visibleKey = visibleKeyBeforeToggle || getVisibleRealPageKeyFromPager(p) || p.activeSection || null;
      ...
      if(visibleKey === "full"){
        const t = pendingFullEditorAnchor || getFirstVisibleFullPerfTarget();
        if(t && t.secKey){
...
function getFirstVisibleFullEditorTarget(){
  // ✅ When Scroll is turned on from FULL view, anchor from the editor that is
  // currently visible BEFORE the editor hides and the perf view shows.
  // This keeps the start point tied to the top-most visible lyric block instead
  // of a seemingly random later spot.
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return null;
    ...
    const editors = Array.from(fullPage.querySelectorAll(".fullSectionEditor[data-sec-editor]"));
    ...
    if(!bestEditor) bestEditor = editors[0];
    if(!bestEditor) return null;

    const secKey = bestEditor.dataset.secEditor || null;
    if(!secKey) return null;

    const cs = getComputedStyle(bestEditor);
    const lineHeight = Math.max(1, parseFloat(cs.lineHeight) || (parseFloat(cs.fontSize) || 18) * 1.45);
    const padTop = parseFloat(cs.paddingTop) || 0;
    const r = bestEditor.getBoundingClientRect();
    const offsetPx = Math.max(0, viewTop - r.top - padTop);
    const visibleLineIdx = Math.max(0, Math.floor(offsetPx / lineHeight));

    const text = String(bestEditor.value || "").replace(/\r/g, "");
    ...
    const flushBlock = () => {
      if(!buf.length || found) return;
      if(visibleLineIdx <= lineCursor + buf.length - 1){
        found = true;
        return;
      }
      lineCursor += buf.length;
      barIdx += 1;
      buf = [];
    };
    ...
    for(const ln of rawLines){
      if(String(ln).trim() === ""){
        flushBlock();
        if(found) break;
        lineCursor += 1; // blank separator line between bars in FULL editor
      }else{
        buf.push(ln);
      }
    }
    ...
    const sec = getActiveProject()?.sections?.[secKey];
    const maxIdx = Math.max(0, ((sec?.bars?.length || 1) - 1));
    return { secKey, barIdx: Math.max(0, Math.min(maxIdx, barIdx)) };
  }catch(_e){
    return null;
  }
}
