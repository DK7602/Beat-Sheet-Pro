...
function renderStickyPageHeader(pageKeyOverride=null){
  ...
  const prevKey = dock.getAttribute("data-page-key") || "";
  dock.setAttribute("data-page-key", key);

  let row = dock.querySelector(":scope > .pageTitleRow.stickyTop");
  if(!row){
    dock.innerHTML = "";
  ...
  if(prevKey === key && row.__boundKey === key){
    const existingInput = row.querySelector('input.sectionPill[data-sec-title]');
    if(existingInput){
      const sec = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:true });
      const nextVal = (sec.title || "").trim();
      if(document.activeElement !== existingInput && existingInput.value !== nextVal){
        existingInput.value = nextVal;
      }
    }
    return;
  }
...
function setupCarouselPager(pagerEl, p){
  ...
  let pendingSnapIdx = null;
  let isProgrammaticSnap = false;
  let lastCommittedIdx = getCurrentIdx(pagerEl);
  ...
  const scheduleSettle = () => {
    ...
    settleTimer = setTimeout(()=>{
      const targetIdx = pendingSnapIdx == null ? getCurrentIdx(pagerEl) : pendingSnapIdx;
      settlePagerToIdx(targetIdx);
    }, 170);
  };
