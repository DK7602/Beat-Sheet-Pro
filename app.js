...
const NOTES_UI_STATE_KEY = `${KEY_PREFIX}notes_ui_state_v1`;

function saveNotesUiState(){
  try{
    const payload = { open: !!notesOpen, ts: Date.now() };
    sessionStorage.setItem(NOTES_UI_STATE_KEY, JSON.stringify(payload));
  }catch(_e){}
}
function clearNotesUiState(){
  try{ sessionStorage.removeItem(NOTES_UI_STATE_KEY); }catch(_e){}
}
function loadNotesUiState(){
  try{
    const raw = sessionStorage.getItem(NOTES_UI_STATE_KEY);
...
(async function boot(){
  ...
  renderAll();
  syncHeaderHeightVar();
  attachPullToRefresh(els.barsScroller, ()=>{ if(notesOpen) saveNotesUiState(); location.reload(); });
