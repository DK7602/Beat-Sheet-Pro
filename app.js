...
let mediaFocusEl = null;
let mediaFocusWanted = false;
let mediaFocusStarted = false;
...
function buildSilentWavDataUrl(durationSec = 1){
  try{
    const sampleRate = 8000;
    const channels = 1;
    const bitsPerSample = 16;
    const samples = Math.max(1, Math.round(sampleRate * Math.max(0.25, Number(durationSec || 1))));
...
function ensureMediaFocusEl(){
  try{
    if(mediaFocusEl) return mediaFocusEl;
    ...
    el.volume = 0.001;
    el.src = buildSilentWavDataUrl(1);
...
function updateMediaSessionPlaybackState(){
  try{
    ...
    navigator.mediaSession.playbackState = mediaFocusWanted ? "playing" : "none";
    if(mediaFocusWanted){
      navigator.mediaSession.metadata = new MediaMetadata({
        title: "Beat Sheet Pro",
        artist: "Beat Sheet Pro"
      });
    }else{
...
async function syncMediaFocusNow(){
  mediaFocusWanted = !!(metroOn || recording || playback.isPlaying);
  updateMediaSessionPlaybackState();
  const el = ensureMediaFocusEl();
  if(!el) return;

  if(mediaFocusWanted){
    try{
      if(!mediaFocusStarted){
        const p = el.play();
        if(p && typeof p.then === "function") await p;
        mediaFocusStarted = !el.paused;
      }else if(el.paused){
        const p = el.play();
        if(p && typeof p.then === "function") await p;
      }
...
function syncMediaFocusSoon(){
  Promise.resolve().then(()=>syncMediaFocusNow()).catch(()=>{});
}
function primeMediaControls(){
  try{ syncMediaFocusSoon(); }catch(_e){}
}
...
async function prewarmRecordingPath(){
  try{
    ensureAudio();
    if(audioCtx?.state === "suspended") await audioCtx.resume();
    primeMediaControls();
...
async function ensureMic(){
  ...
  try{
    micStream = await navigator.mediaDevices.getUserMedia({ audio: cleanMusicConstraints });
  }catch(err){
...
function cleanupMic(){
  try{ micSource?.disconnect?.(); }catch(_e){}
  try{ micGain?.disconnect?.(); }catch(_e){}
  try{
...
// record button
els.recordBtn?.addEventListener("pointerdown", ()=>{ if(!recording) prewarmRecordingPath(); }, { passive:true });
els.recordBtn?.addEventListener("touchstart", ()=>{ if(!recording) prewarmRecordingPath(); }, { passive:true });
els.recordBtn?.addEventListener("mousedown", ()=>{ if(!recording) prewarmRecordingPath(); });
