...
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      // Keep the top window locked to the drum row, not to growing recording content.
      const sticky = header.querySelector(".stickyTools") || document.querySelector(".stickyTools");
      if(sticky){
        const r = sticky.getBoundingClientRect();
        h = Math.ceil(r.bottom);
      }else{
...
const MASTER_MIX_NORMAL_GAIN = 0.95;
const MASTER_MIX_RECORDING_GAIN = 0.42;
...
masterMix.gain.value = MASTER_MIX_NORMAL_GAIN; // tiny headroom
...
function updateRecordingMonitorState(){
  try{
    if(masterMix){
      const wantsMonitorDuck = !!(recording && (metroOn || playback?.isPlaying));
      masterMix.gain.cancelScheduledValues(audioCtx?.currentTime || 0);
      masterMix.gain.value = wantsMonitorDuck ? MASTER_MIX_RECORDING_GAIN : MASTER_MIX_NORMAL_GAIN;
    }
  }catch(_e){}
}
...
async function refreshMicProcessingProfile(){
  ...
  if(micGain){
    ...
    micGain.gain.value = wantsEchoProtection ? 0.09 : 0.15;
  }
}
