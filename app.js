...
function createNotesTodoLine(html="<br>", checked=false){
  ...
  text.className = NOTES_TODO_TEXT_CLASS;
  text.innerHTML = String(html || "").trim() ? html : notesBlankTypingHtml();
...
function notesTodoTextValue(line){
  const text = ensureNotesTodoText(line);
  return String(text?.innerText || text?.textContent || "").replace(/ /g, " ").trim();
}
...
function insertChecklistIntoNotes(){
  ...
  if(!notesTodoTextValue(todo)) seedNotesTypingTarget(text, getNotesTypingColor(), true);
  else setCaretAtStartOfNode(text);
  saveNotesFromEditor();
  ...
  ed.addEventListener("keydown", (e)=>{
    ...
    if(e.key === "Enter"){
      ...
      if(value){
        ...
        seedNotesTypingTarget(nextText, getNotesTypingColor(), true);
      }else{
