...
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      ...
      const hsRect = hs?.getBoundingClientRect?.() || headerRect;
      const padTop = parseFloat(getComputedStyle(hs || header).paddingTop || "0") || 0;
      const padBottom = parseFloat(getComputedStyle(hs || header).paddingBottom || "0") || 0;
      ...
      if(sticky){
        ...
        rectBottom = Math.ceil(stickyRect.bottom - headerRect.top);
        const stickyOffsetTop = Number.isFinite(sticky.offsetTop) ? sticky.offsetTop : 0;
        fallbackBottom = Math.ceil((hsRect.top - headerRect.top) + stickyOffsetTop + sticky.offsetHeight + padBottom);
      }

      const headerBox = Math.ceil(header.offsetHeight || headerRect.height || 0);
      const scrollBox = Math.ceil((hs?.scrollHeight || 0) + padTop + padBottom);
      h = Math.max(rectBottom, fallbackBottom, headerBox, scrollBox, 140);

      if(lastExpandedHeaderHeight && h < Math.max(96, Math.floor(lastExpandedHeaderHeight * 0.55))){
        h = lastExpandedHeaderHeight;
      }
      lastExpandedHeaderHeight = h;
