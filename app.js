...
function renderProjectPicker(){
  ...
  const label = activeIsArchived ? "Projects" : (projects.length ? getProjectDisplayName(active) : "No projects");

  els.projectPicker.disabled = !projects.length;
  els.projectPicker.textContent = label;
  els.projectPicker.title = label || "Projects";
  els.projectPicker.setAttribute("aria-label", label || "Projects");
  els.projectPicker.dataset.projectCount = String(projects.length || 0);
...
els.projectList?.addEventListener("click", (e)=>{
  ...
  if(!id) return;
  if(setActiveProjectById(id)) closeProjectOverlay();
});
