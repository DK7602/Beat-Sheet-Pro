...
function preserveMainViewport(fn){
  const scroller = els?.barsScroller || document.getElementById("bars");
  const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
  const top = scroller ? scroller.scrollTop : 0;
  const left = pagerEl ? pagerEl.scrollLeft : 0;

  try{ if(fn) fn(); }catch(err){ throw err; }

  const restore = ()=>{
    try{ if(scroller) scroller.scrollTop = top; }catch(_e){}
    try{ if(pagerEl) pagerEl.scrollLeft = left; }catch(_e){}
  };
...
function getVisibleRealPageKeyFromPager(p){
  try{
    ...
    const probeX = pagerRect.left + (pagerRect.width / 2);
    const probeY = pagerRect.top + Math.min(24, Math.max(8, pagerRect.height * 0.12));
    const hit = document.elementFromPoint(probeX, probeY);
    const hitPage = hit && hit.closest ? hit.closest('.page[data-page-key]') : null;
    if(hitPage && hitPage.dataset && hitPage.dataset.pageKey){
      return hitPage.dataset.pageKey;
    }
...
function computeSeqStartOffsetFromAnchor(p, pageKey, barIdx){
  try{
    ...
    let off = seq.findIndex(x => (x.secKey === k || x.sectionKey === k) && Number(x.barIdx) === Number(i));
    if(off >= 0) return off;

    off = seq.findIndex(x => (x.secKey === k || x.sectionKey === k) && Number(x.barIdx) >= Number(i));
    if(off >= 0) return off;

    const order = getFullOrder(p);
    const startOrderIdx = Math.max(0, order.indexOf(k));
    for(let step = 1; step < order.length; step++){
      const secKey = order[(startOrderIdx + step) % order.length];
      off = seq.findIndex(x => (x.secKey === secKey || x.sectionKey === secKey));
      if(off >= 0) return off;
    }
