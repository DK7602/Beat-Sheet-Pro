...
function setChecklistRowChecked(row, checked){
  try{
    ...
    if(box) box.setAttribute('aria-label', checked ? 'Checked' : 'Unchecked');
  }catch(_e){}
}
...
function insertChecklistRowAtSelection(){
  try{
    ...
    block = block?.closest?.('div, p, li');

    if(block && block !== ed && ed.contains(block)){
      block.insertAdjacentElement('afterend', newRow);
    }else{
      range.collapse(false);
  ...
  ed.addEventListener("beforeinput", (e)=>{
    ...
    if(e.inputType === "deleteContentBackward"){
      ...
      if(range && range.collapsed){
        const atStart = (range.startContainer === textEl && range.startOffset === 0) ||
          (range.startContainer.nodeType === Node.TEXT_NODE && textEl.contains(range.startContainer) && range.startOffset === 0 && !range.startContainer.previousSibling);
        if(atStart || isNotesNodeEmpty(textEl)){
          e.preventDefault();
