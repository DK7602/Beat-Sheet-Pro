...
function makeNotesTypingMarkerSpan(color){
...
function sanitizeNotesHtmlForRender(html){
  try{
    const wrap = document.createElement("div");
    wrap.innerHTML = String(html || "");

    wrap.querySelectorAll('.notesCheckRow').forEach((row)=>{
      const legacyInput = row.querySelector('.notesCheckBox');
      const legacyButton = row.querySelector('.notesCheckToggle');
      const checked = !!(row.classList.contains('isChecked') || legacyInput?.checked || legacyInput?.hasAttribute('checked') || legacyButton?.getAttribute('aria-checked') === 'true');
      row.classList.toggle('isChecked', checked);

      if(legacyInput){
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'notesCheckToggle';
        btn.setAttribute('role', 'checkbox');
        btn.setAttribute('aria-label', 'Checklist item');
        btn.contentEditable = 'false';
        btn.tabIndex = -1;
        legacyInput.replaceWith(btn);
      }

      const btn = row.querySelector('.notesCheckToggle');
      if(btn){
        btn.type = 'button';
        btn.classList.add('notesCheckToggle');
        btn.setAttribute('role', 'checkbox');
        btn.setAttribute('aria-label', 'Checklist item');
      ...
      const txt = row.querySelector('.notesCheckText');
      if(txt){
        txt.contentEditable = 'true';
        txt.spellcheck = true;
        if(!normalizeNotesHtml(txt.innerHTML)) txt.innerHTML = '<br>';
        if(!checked){
    ...
    wrap.querySelectorAll('.notesPlainLine').forEach((line)=>{
      line.contentEditable = 'true';
      if(!normalizeNotesHtml(line.innerHTML)) line.innerHTML = '<br>';
    });
...
function setNotesCheckRowState(row, checked){
  try{
    if(!row) return;
    const on = !!checked;
    row.classList.toggle('isChecked', on);
    const btn = row.querySelector('.notesCheckToggle');
...
function primeNotesTypingColor(color){
  try{
    ...
    const plainText = String(ed.textContent || "").replace(/​/g, "").trim();
    if(!plainText){
      ed.innerHTML = "";
      const span = makeNotesTypingMarkerSpan(c);
      const marker = document.createTextNode("​");
      span.appendChild(marker);
      ed.appendChild(span);

      const emptyRange = document.createRange();
      emptyRange.setStart(marker, marker.nodeValue.length);
      emptyRange.collapse(true);
      sel.removeAllRanges();
      sel.addRange(emptyRange);
      lastNotesRange = emptyRange.cloneRange();
      return;
    }
...
function enforceNotesTypingStyleForEmptyEditor(){
  try{
    const ed = els.notesEditor;
    if(!notesOpen || !ed) return;
    const text = String(ed.textContent || "").replace(/​/g, "").trim();
    const hasCheckRows = !!ed.querySelector('.notesCheckRow');
    if(text || hasCheckRows) return;
    primeNotesTypingColor(getNotesTypingColor());
  }catch(_e){}
}

function makeNotesCheckRow(textHtml="", checked=false){
  const row = document.createElement("div");
  row.className = "notesCheckRow";

  const box = document.createElement("button");
  box.type = "button";
  box.className = "notesCheckToggle";
  ...
  txt.className = "notesCheckText";
  txt.contentEditable = "true";
  txt.spellcheck = true;
  txt.innerHTML = textHtml || "";
  if(!txt.innerHTML) txt.innerHTML = "<br>";

  row.appendChild(box);
  row.appendChild(txt);
  setNotesCheckRowState(row, checked);
  return row;
}

function focusNotesCheckText(el){
  try{
    if(!el) return;
    if(el.nodeType === Node.ELEMENT_NODE && !el.isContentEditable) el.contentEditable = "true";
    el.focus();
    const sel = window.getSelection?.();
    if(!sel) return;
...
function insertChecklistAtCurrentNotesPosition(){
  try{
    const ed = els.notesEditor;
    if(!notesOpen || !ed) return;
    ...
    const row = makeNotesCheckRow("", false);
    const sel = window.getSelection?.();
    let anchor = null;
    ...
    if(sel && sel.rangeCount){
      ...
      if(node && ed.contains(node)){
        anchor = (node.nodeType === Node.ELEMENT_NODE ? node : node.parentNode);
      }
    }

    if(!anchor && document.activeElement && ed.contains(document.activeElement)){
      anchor = document.activeElement;
    }

    const currentText = anchor?.closest?.('.notesCheckText');
    const currentRow = currentText?.closest?.('.notesCheckRow');
    if(currentRow && currentRow.parentNode){
      currentRow.parentNode.insertBefore(row, currentRow.nextSibling);
      focusNotesCheckText(row.querySelector('.notesCheckText'));
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
      return;
    }

    const currentPlain = anchor?.closest?.('.notesPlainLine');
    if(currentPlain && currentPlain.parentNode){
      currentPlain.parentNode.insertBefore(row, currentPlain.nextSibling);
      focusNotesCheckText(row.querySelector('.notesCheckText'));
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
      return;
    ...
    const meaningful = String(ed.textContent || '').replace(/​/g, '').trim();
    const hasRows = !!ed.querySelector('.notesCheckRow, .notesPlainLine');
    if(!meaningful && !hasRows){
      ed.innerHTML = '';
      ed.appendChild(row);
      focusNotesCheckText(row.querySelector('.notesCheckText'));
      saveNotesFromEditor();
    ...
    let block = anchor?.closest?.('div, p, li, ul, ol, blockquote, pre, h1, h2, h3, h4, h5, h6');
    if(block && block !== ed && ed.contains(block) && block.parentNode){
...
function convertEmptyCheckRowToPlain(row){
  try{
    if(!row) return;
    const plain = makeNotesPlainLine();
    row.replaceWith(plain);
    focusNotesCheckText(plain);
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
  }catch(_e){}
}
...
function openNotes(){
  ...
  els.notesEditor.innerHTML = sanitizeNotesHtmlForRender(p.notesHtml || "");
  setDockHidden(true);
  updateRhymes("");
...
function wireNotesEditor(){
  ...
  ed.addEventListener("click", (e)=>{
    const box = e.target?.closest?.('.notesCheckToggle');
    if(box){
      e.preventDefault();
      const row = box.closest('.notesCheckRow');
      if(row){
        const next = !row.classList.contains('isChecked');
        setNotesCheckRowState(row, next);
        const txt = row.querySelector('.notesCheckText');
        if(txt) focusNotesCheckText(txt);
      }
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
      return;
  ...
  ed.addEventListener("focusin", (e)=>{
    const txt = e.target?.closest?.('.notesCheckText, .notesPlainLine');
    if(!txt) return;
    applyNotesTypingColor(getNotesTypingColor(), false);
    requestAnimationFrame(()=>{
      try{ document.execCommand("removeFormat", false, null); }catch(_e){}
      applyNotesTypingColor(getNotesTypingColor(), false);
      saveNotesSelection();
    });
  });
  ...
  ed.addEventListener("keydown", (e)=>{
    const txt = e.target?.closest?.('.notesCheckText');
    if(!txt) return;
    const row = txt.closest('.notesCheckRow');
    if(!row) return;

    if(e.key === 'Enter'){
      e.preventDefault();
      const content = String(txt.textContent || '').replace(/​/g, '').trim();
      if(!content){
        const plain = makeNotesPlainLine();
        row.replaceWith(plain);
        focusNotesCheckText(plain);
      }else{
        const newRow = makeNotesCheckRow('', false);
        row.parentNode.insertBefore(newRow, row.nextSibling);
        focusNotesCheckText(newRow.querySelector('.notesCheckText'));
      }
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
      return;
    ...
    if(e.key === 'Backspace'){
      const sel = window.getSelection?.();
      const atStart = !!(sel && sel.rangeCount && sel.getRangeAt(0).collapsed && sel.anchorNode && txt.contains(sel.anchorNode) && sel.anchorOffset === 0 && (sel.anchorNode === txt || sel.anchorNode.parentNode === txt || txt.textContent === ''));
      const content = String(txt.textContent || '').replace(/​/g, '').trim();
      if(!content && atStart){
        e.preventDefault();
        const plain = makeNotesPlainLine();
  ...
  ed.addEventListener("beforeinput", (e)=>{
    if(e?.inputType === "deleteContentBackward" || e?.inputType === "deleteContentForward" || e?.inputType === "deleteByCut"){
      requestAnimationFrame(()=>{
        enforceNotesTypingStyleForEmptyEditor();
        applyNotesTypingColor(getNotesTypingColor(), false);
        saveNotesSelection();
      });
    }
  ...
  ed.addEventListener("keyup", ()=>{ enforceNotesTypingStyleForEmptyEditor(); saveNotesSelection(); scheduleNotesRhymeRefresh(); });
  ed.addEventListener("click", ()=>{ enforceNotesTypingStyleForEmptyEditor(); saveNotesSelection(); scheduleNotesRhymeRefresh(); });
  ed.addEventListener("mouseup", ()=>{ enforceNotesTypingStyleForEmptyEditor(); saveNotesSelection(); scheduleNotesRhymeRefresh(); });
  ed.addEventListener("focus", ()=>{ enforceNotesTypingStyleForEmptyEditor(); applyNotesTypingColor(getNotesTypingColor(), false); saveNotesSelection(); scheduleNotesRhymeRefresh(); });
...
if(els.bpm) els.bpm.value = p.bpm || 95;
  ...
  updateAutoScrollBtn();
  wireNotesEditor();
  if(!notesOpen && els.notesEditor) els.notesEditor.innerHTML = sanitizeNotesHtmlForRender(p.notesHtml || "");
...
els.notesChecklistBtn?.addEventListener("click", ()=> insertChecklistAtCurrentNotesPosition());
