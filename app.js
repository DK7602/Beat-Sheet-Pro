...
function getMidpointIdx(pagerEl){
  const w = measurePager(pagerEl);
  const idx = Math.floor((pagerEl.scrollLeft + (w * 0.5)) / w);
  return Math.max(0, Math.min((pagerEl.children.length - 1), idx));
}
function keyFromCarouselIdx(p, idx){
  const order = getActivePageOrder(p);
  const carousel = [order[order.length - 1], ...order, order[0]];
  let key = carousel[idx] || "full";
  if(idx === 0) key = order[order.length - 1];
  if(idx === carousel.length - 1) key = order[0];
  return key || "full";
}
function buildDockTitleRow(p, key){
  ...
  if(key === "full"){
    ...
    titleNode.className = "pageTitle";
    titleNode.textContent = "Full Song View";
  }else{
    const sec = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:true });
    titleNode = document.createElement("input");
    titleNode.type = "text";
    titleNode.spellcheck = false;
    titleNode.className = "pageTitlePill sectionPill";
    titleNode.placeholder = "Song Part";
    titleNode.value = (sec.title || "").trim();
    titleNode.setAttribute("data-sec-title", key);
    titleNode.addEventListener("input", ()=>{
      sec.title = titleNode.value;
      document.querySelectorAll(`input.sectionPill[data-sec-title="${key}"]`).forEach(inp=>{
        if(inp !== titleNode) inp.value = titleNode.value;
      });
      touchProject(p);
    });
  }
...
function renderPageHeaderDock(p, key){
  const dock = els?.pageHeaderDock || document.getElementById("pageHeaderDock");
  if(!dock) return;
  const safeKey = key || p?.activeSection || "full";
  if(dock.dataset.pageKey === safeKey) return;
  dock.dataset.pageKey = safeKey;
  dock.innerHTML = "";
  dock.appendChild(buildDockTitleRow(p, safeKey));
}
...
function setActiveSectionFromIdx(p, idx, opts={}){
  const key = keyFromCarouselIdx(p, idx);
  const persist = opts.persist !== false;
  const closeUi = opts.closeUi !== false;

  if(p.activeSection !== key){
    p.activeSection = key;
    if(persist) touchProject(p);
...
function setupCarouselPager(pagerEl, p){
  ...
  if(pagerEl.__resizeSnapHandler){
    window.removeEventListener("resize", pagerEl.__resizeSnapHandler);
  }
  ...
  const snapToActive = () => {
    ...
    renderPageHeaderDock(p, p.activeSection || "full");
  };
  pagerEl.__resizeSnapHandler = snapToActive;
  ...
  const liveUpdate = () => {
    ticking = false;
    const midIdx = getMidpointIdx(pagerEl);
    setActiveSectionFromIdx(p, midIdx, { persist:false, closeUi:false });
  };

  const handleSettled = () => {
    const order = getActivePageOrder(p);
    const carousel = [order[order.length - 1], ...order, order[0]];
    let idx = getCurrentIdx(pagerEl);
    ...
    setActiveSectionFromIdx(p, idx, { persist:true, closeUi:true });
  };
  ...
  pagerEl.addEventListener("scroll", ()=>{
    ...
    settleTimer = setTimeout(handleSettled, 70);
  }, { passive:true });
...
const mountEl = pageEl ? pageEl.querySelector(".sectionMount") : null;
  ...
  renderPageHeaderDock(p, wantKey);
