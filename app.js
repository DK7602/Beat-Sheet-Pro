...
function getAutoScrollViewportForPage(pageEl){
  const fallbackScroller = (typeof findVerticalScroller === "function")
    ? (findVerticalScroller(pageEl) || document.scrollingElement || document.documentElement)
    : (document.scrollingElement || document.documentElement);

  const scroller = (els?.barsScroller && els.barsScroller.contains(pageEl))
    ? els.barsScroller
    : fallbackScroller;
  ...
  const sticky = pageEl.querySelector(".pageTitleRow, .stickyTop");
  if(sticky){
    const sr = sticky.getBoundingClientRect();
    if(Number.isFinite(sr.bottom)) viewTop = Math.max(viewTop, sr.bottom + 4);
  }

  return { scroller, viewTop, viewBottom };
}

function getFirstVisibleBarIdxInActivePage(){
  // ✅ Start from the FIRST visible card in the bottom window, including a card
  // that is partially visible at the top edge.
  try{
    const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
    ...
    const { viewTop, viewBottom } = getAutoScrollViewportForPage(pageEl);

    for(const el of bars){
      const r = el.getBoundingClientRect();
      const isVisible = r.bottom > (viewTop + 1) && r.top < (viewBottom - 1);
...
function getFirstVisibleFullPerfTarget(){
  // ✅ Start from the FIRST visible full-song performance line in the bottom window,
  // including a line that is partially visible at the top edge.
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return null;
    ...
    const { viewTop, viewBottom } = getAutoScrollViewportForPage(fullPage);
    ...
    for(const el of lines){
      ...
      const idx = parseInt(el.getAttribute("data-bar-idx") || el.dataset.barIdx || "0", 10);
      return { secKey: secKey || null, barIdx: Number.isFinite(idx) ? idx : 0 };
    }

    const first = lines[0];
    const firstSec = first.closest(".fullSection");
    const firstSecKey = firstSec ? (firstSec.dataset.secKey || firstSec.getAttribute("data-sec-key")) : null;
    const firstIdx = parseInt(first.getAttribute("data-bar-idx") || first.dataset.barIdx || "0", 10);
    return { secKey: firstSecKey || null, barIdx: Number.isFinite(firstIdx) ? firstIdx : 0 };
  }catch(_e){
    return null;
  }
}
...
function getVisibleRealPageKeyFromPager(p){
  try{
    ...
    const realPages = Array.from(pagerEl.querySelectorAll('.page[data-page-key]:not([data-clone="1"])'));
    if(realPages.length){
      const pagerRect = pagerEl.getBoundingClientRect();
      const pagerMidX = pagerRect.left + (pagerRect.width / 2);

      let bestKey = null;
      let bestVisibleWidth = -1;
      let bestCenterDelta = Infinity;

      for(const pageEl of realPages){
        const rect = pageEl.getBoundingClientRect();
        const visibleLeft = Math.max(rect.left, pagerRect.left);
        const visibleRight = Math.min(rect.right, pagerRect.right);
        const visibleWidth = Math.max(0, visibleRight - visibleLeft);
        if(visibleWidth <= 0) continue;

        const pageMidX = rect.left + (rect.width / 2);
        const centerDelta = Math.abs(pageMidX - pagerMidX);
        const key = pageEl.dataset.pageKey || null;
        if(!key) continue;

        const wins = (visibleWidth > bestVisibleWidth + 1)
          || (Math.abs(visibleWidth - bestVisibleWidth) <= 1 && centerDelta < bestCenterDelta);
