...
function getFirstVisibleBarIdxInActivePage(){
  // ✅ Start from the FIRST bar/card that is visible in the viewport.
  // This includes a bar whose top is slightly above the viewport edge,
  // which matches the user's "top most line in view" expectation.
  try{
    const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
    ...
    for(const el of bars){
      ...
      const visible = r.bottom > (viewTop + 2) && r.top < (viewBottom - 2);
      if(!visible) continue;
...
function getFirstVisibleFullPerfTarget(){
  // ✅ Start from the FIRST lyric line that is visible in the FULL performance view.
  // A line that is partially visible at the top still counts as "top most line in view".
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return null;
