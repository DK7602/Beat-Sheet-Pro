...
const RHYME_LYRIC_BOOST_WORDS = new Set([
  "love","heart","baby","night","light","fire","desire","higher","alone","home","gone","strong","weak",
  "pain","rain","grace","faith","praise","glory","mercy","hope","dream","feel","feeling","heal","healing",
  "broken","hold","touch","fall","rise","run","stay","away","today","tonight","forever","never","maybe",
  "crazy","save","saved","savior","god","lord","jesus","soul","road","way","name","dance","sing","song",
  "time","mind","find","shine","sky","stars","shadow","chance","change","free","freedom","believe","belong"
]);
const RHYME_OBSCURE_PENALTY_RE = /(?:phobia|logy|graphy|hedron|esque|arium|orium|aceous|ation|ologist|escent)$/i;
...
function scoreRhymeCandidate(seed, item, kind){
  ...
  if(kind === 'near') score += 70;

  const sSyl = syllableEstimate(seed);
  const wSyl = syllableEstimate(word);
  score += Math.max(0, 18 - (Math.abs(sSyl - wSyl) * 6));

  if(RHYME_LYRIC_BOOST_WORDS.has(word)) score += 45;
  if(word.length >= 3 && word.length <= 8) score += 14;
  if(word.length > 10) score -= 16;
  if(/^(un|re|mis|dis)/.test(word) && word.length > 8) score -= 8;
  if(RHYME_OBSCURE_PENALTY_RE.test(word)) score -= 40;
  if(/[^a-z']/.test(word)) score -= 10;
  if(/(?:ness|ment|tion|sion|tude|ship|ology|able|ible)$/.test(word) && !RHYME_LYRIC_BOOST_WORDS.has(word)) score -= 10;

  const numSyllables = Number(item?.numSyllables || 0);
  if(numSyllables){
    score += Math.max(0, 14 - (Math.abs(numSyllables - sSyl) * 4));
  }
  const freq = Number(item?.score || 0);
  score += Math.max(0, Math.min(24, Math.log10(Math.max(1, freq + 1)) * 10));

  if(rhymeTail(word) === rhymeTail(seed)) score += 12;
  if(vowelTail(word) === vowelTail(seed)) score += 8;
...
function buildRhymeBuckets(seed, perfectData, nearData){
  ...
  const near = dedupeRankedWords((nearData||[]).map(item => ({
    word: item.word,
    score: scoreRhymeCandidate(seed, item, 'near')
  })).filter(item => !perfectSet.has(normalizeRhymeWord(item.word))))
    .sort((a,b)=>b.score-a.score || a.word.localeCompare(b.word));

  return {
    perfect: perfect.slice(0, 10).map(x=>x.word),
    near: near.slice(0, 8).map(x=>x.word)
  };
}
...
async function updateRhymes(seed){
  ...
  try{
    ...
    const [perfectRes, nearRes] = await Promise.all([
      fetch(`https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&max=28`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&max=24`, { signal: rhymeAbort.signal })
    ]);
    const perfectData = await perfectRes.json();
    const nearData = await nearRes.json();
    if(myReq !== rhymeRequestId) return;

    const buckets = buildRhymeBuckets(w, perfectData, nearData);
