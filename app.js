...
function renderProjectOverlayList(){
...
function setActiveProjectById(projectId){
  const id = String(projectId || "").trim();
  if(!id) return false;
  const found = (store.projects || []).find(p => p.id === id && !isArchivedProject(p));
  if(!found) return false;
  store.activeProjectId = id;
  found.activeSection = "full";
  saveStoreSafe();
  playback.stop(false);
  closeProjectOverlay();
  renderAll();
...
function wireProjectOverlayOpeners(){
  if(!els.projectPicker || els.projectPicker.__wiredOpeners) return;
  els.projectPicker.__wiredOpeners = true;
  const openFromButton = (e)=>{
    if(e){ try{ e.preventDefault(); }catch(_e){} try{ e.stopPropagation(); }catch(_e){} }
    if(els.projectPicker.disabled) return;
    openProjectOverlay();
  };
  els.projectPicker.addEventListener("click", openFromButton);
  els.projectPicker.addEventListener("touchend", openFromButton, { passive:false });
  els.projectPicker.addEventListener("pointerup", (e)=>{
    if(e.pointerType === "touch") openFromButton(e);
  });
  els.projectPicker.addEventListener("keydown", (e)=>{
    if(e.key === "Enter" || e.key === " ") openFromButton(e);
  });
}
...
els.projectPicker?.addEventListener("contextmenu", (e)=>{ try{ e.preventDefault(); }catch(_e){} });
els.projectCloseBtn?.addEventListener("click", ()=> closeProjectOverlay());
els.projectOverlay?.addEventListener("click", (e)=>{ if(e.target === els.projectOverlay) closeProjectOverlay(); });
...
els.archiveMoveList?.addEventListener("click", (e)=>{
  ...
  if(moved && moved.id === store.activeProjectId){
    const next = getRegularProjects()[0] || moved;
...
els.archiveList?.addEventListener("click", (e)=>{
  ...
  const found = (store.projects || []).find(p => p.id === id && isArchivedProject(p));
  if(!found) return;
  store.activeProjectId = found.id;
  found.activeSection = "full";
  saveStoreSafe();
  playback.stop(false);
  closeArchive();
...
const startArchiveLongPress = (target)=>{
  clearTimeout(archiveLongPressTimer);
  const btn = target?.closest?.("[data-archive-id]");
  if(!btn) return;
  const id = btn.getAttribute("data-archive-id");
  archiveLongPressTimer = setTimeout(()=> openArchiveConfirm(id), 550);
};
const clearArchiveLongPress = ()=>{
  if(archiveLongPressTimer) clearTimeout(archiveLongPressTimer);
  archiveLongPressTimer = null;
};
els.archiveList?.addEventListener("pointerdown", (e)=>{ if(e.pointerType === "touch") startArchiveLongPress(e.target); });
els.archiveList?.addEventListener("pointerup", clearArchiveLongPress);
els.archiveList?.addEventListener("pointercancel", clearArchiveLongPress);
els.archiveList?.addEventListener("pointerleave", clearArchiveLongPress);
els.archiveList?.addEventListener("contextmenu", (e)=>{
  const btn = e.target.closest("[data-archive-id]");
  if(!btn) return;
  e.preventDefault();
...
els.archiveConfirmMoveBtn?.addEventListener("click", ()=>{
  ...
  if(moved) store.activeProjectId = moved.id;
  closeArchiveConfirm();
  renderAll();
