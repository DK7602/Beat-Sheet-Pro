...
const els = {
  ...
  notesCropLeft: need("notesCropLeft"),
  notesCropTop: need("notesCropTop"),
  notesCropRight: need("notesCropRight"),
  notesCropBottom: need("notesCropBottom"),
...
function selectNotesPhoto(item){
  ...
  if(selectedNotesPhoto?.classList) selectedNotesPhoto.classList.remove(NOTES_PHOTO_SELECTED_CLASS);
  selectedNotesPhoto = item || null;
  if(selectedNotesPhoto?.classList) selectedNotesPhoto.classList.add(NOTES_PHOTO_SELECTED_CLASS);
}
...
function makeNotesPhotoItem(src, opts={}){
  ...
  wrap.innerHTML = `
    ...
    <div class="notesPhotoBtns" contenteditable="false">
      <button type="button" class="notesPhotoMiniBtn notesPhotoCropBtn" title="Crop" aria-label="Crop photo">Crop</button>
...
function normalizeNotesPhotoStructure(root=els?.notesEditor){
  ...
  root.querySelectorAll('[data-notes-photo="1"]').forEach((item)=>{
    ...
    if(!btns){
      ...
      btns.innerHTML = '<button type="button" class="notesPhotoMiniBtn notesPhotoCropBtn" title="Crop" aria-label="Crop photo">Crop</button><button type="button" class="notesPhotoMiniBtn notesPhotoDeleteBtn" title="Delete" aria-label="Delete photo">×</button>';
...
function getNotesCropValues(){
  return {
    left: clampNotesPhoto(els.notesCropLeft?.value, 0, 95),
    top: clampNotesPhoto(els.notesCropTop?.value, 0, 95),
    right: clampNotesPhoto(els.notesCropRight?.value, 0, 95),
    bottom: clampNotesPhoto(els.notesCropBottom?.value, 0, 95)
  };
}
function syncNotesCropBox(){
  const wrap = els.notesCropPreviewWrap;
  const box = els.notesCropBox;
  if(!wrap || !box) return;
  const vals = getNotesCropValues();
  const maxH = Math.max(0, 100 - vals.top - vals.bottom);
  const maxW = Math.max(0, 100 - vals.left - vals.right);
  box.style.left = vals.left + '%';
  box.style.top = vals.top + '%';
  box.style.width = maxW + '%';
...
function openNotesCropOverlay(item){
  ...
  if(els.notesCropPreviewImg) els.notesCropPreviewImg.src = src;
  if(els.notesCropLeft) els.notesCropLeft.value = '0';
  if(els.notesCropTop) els.notesCropTop.value = '0';
  if(els.notesCropRight) els.notesCropRight.value = '0';
  if(els.notesCropBottom) els.notesCropBottom.value = '0';
  syncNotesCropBox();
...
async function applyNotesCropOverlay(){
  ...
  if(!item || !src) return;
  const vals = getNotesCropValues();
  if((vals.left + vals.right) >= 100 || (vals.top + vals.bottom) >= 100){
    showToast('Crop too large');
    return;
  }
  try{
    const img = await loadImageFromSrc(src);
    ...
    const sx = Math.round(sw * vals.left / 100);
    const sy = Math.round(sh * vals.top / 100);
    const cw = Math.max(1, Math.round(sw * (100 - vals.left - vals.right) / 100));
    const ch = Math.max(1, Math.round(sh * (100 - vals.top - vals.bottom) / 100));
    const canvas = document.createElement('canvas');
    canvas.width = cw;
    canvas.height = ch;
    const ctx = canvas.getContext('2d');
...
function finishNotesPhotoPointer(e){
  ...
  if(drag.kind === 'drag' || drag.kind === 'resize') saveNotesFromEditor();
  clearNotesPhotoPointer();
}
...
[els.notesCropLeft, els.notesCropTop, els.notesCropRight, els.notesCropBottom].forEach((el)=>{
  el?.addEventListener("input", syncNotesCropBox);
});
els.notesCropCancelBtn?.addEventListener("click", ()=> closeNotesCropOverlay());
els.notesCropApplyBtn?.addEventListener("click", ()=> applyNotesCropOverlay());
els.notesCropOverlay?.addEventListener("click", (e)=>{ if(e.target === els.notesCropOverlay) closeNotesCropOverlay(); });
...
document.addEventListener("click", (e)=>{ if(notesOpen && !e.target?.closest?.("[data-notes-photo=\"1\"]")) clearSelectedNotesPhoto(); });
