...
function setAutoScroll(v){
  ...
  if(turningOn){
    // Anchor to the top-most visible line/card currently in view (do not jump back to page 1).
    // Measure after the auto-scroll classes are applied so FULL view uses the visible perf lines,
    // not the pre-toggle editor layout.
    if(p){
      const anchor = getAutoScrollViewportAnchor(p);
      const visibleKey = anchor?.pageKey || getVisibleRealPageKeyFromPager(p) || p.activeSection || null;
      ...
      p.playback.seqStartOffset = Number.isFinite(+anchor?.seqOffset) ? (+anchor.seqOffset) : 0;
    }
...
function getAutoScrollViewportAnchor(p){
  try{
    const proj = p || ((typeof getActiveProject === "function") ? getActiveProject() : null);
    const visibleKey = getVisibleRealPageKeyFromPager(proj) || proj?.activeSection || "full";

    if(visibleKey === "full"){
      const fullPage = (typeof getActiveRealPageEl === "function") ? getActiveRealPageEl("full") : null;
      if(fullPage){
        // Force layout after .autoScrollOn/.autoScrollPaused classes change so we measure
        // the performance lines in their final visible positions, not the pre-toggle editor layout.
        void fullPage.offsetHeight;
      }
      ...
      if(t && t.secKey){
        return {
          pageKey: "full",
          secKey: t.secKey,
          barIdx: Number.isFinite(+t.barIdx) ? (+t.barIdx) : 0,
          seqOffset: computeSeqStartOffsetFromAnchor(proj, t.secKey, t.barIdx)
        };
      }
      return { pageKey: "full", secKey: null, barIdx: 0, seqOffset: 0 };
    }

    const pageEl = (typeof getActiveRealPageEl === "function") ? getActiveRealPageEl(visibleKey) : null;
    if(pageEl){
      void pageEl.offsetHeight;
    }
    const barIdx = getFirstVisibleBarIdxInActivePage();
    return {
      pageKey: visibleKey,
      secKey: visibleKey,
      barIdx,
      seqOffset: computeSeqStartOffsetFromAnchor(proj, visibleKey, barIdx)
    };
  }catch(_e){
    return {
      pageKey: (p && p.activeSection) ? p.activeSection : "full",
      secKey: (p && p.activeSection && p.activeSection !== "full") ? p.activeSection : null,
      barIdx: 0,
      seqOffset: 0
    };
  }
...
function startPracticeScroll(p, opts={}){
  ...
  if(activeProject){
    ...
    if(!preserveAnchor){
      const anchor = getAutoScrollViewportAnchor(activeProject);
      const visibleKey = anchor?.pageKey || getVisibleRealPageKeyFromPager(activeProject) || activeProject.activeSection || null;
      if(visibleKey){
      ...
      activeProject.playback.seqStartOffset = Number.isFinite(+anchor?.seqOffset) ? (+anchor.seqOffset) : 0;
    }
  }
