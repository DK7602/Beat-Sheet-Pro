...
function saveNotesOpenState(v){
  try{
    if(v) sessionStorage.setItem(NOTES_OPEN_KEY, "1");
    else sessionStorage.removeItem(NOTES_OPEN_KEY);
  }catch(_e){}
}
function loadNotesOpenState(){
  try{ return sessionStorage.getItem(NOTES_OPEN_KEY) === "1"; }catch(_e){ return false; }
}
function refreshAppKeepUi(){
  try{ saveNotesOpenState(!!notesOpen); }catch(_e){}
...
const NOTES_OPEN_KEY = `${KEY_PREFIX}notes_open_v1`;
