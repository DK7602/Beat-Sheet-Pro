...
function buildRhymeBuckets(seed, perfectData, nearData, soundLikeData){
  ...
  return {
    perfect: perfect.slice(0, 30).map(x=>x.word),
    near: near.slice(0, 30).map(x=>x.word)
  };
}
...
async function updateRhymes(seed){
  ...
  if(Array.isArray(cached) && cached.length){
    const fallbackBuckets = { perfect: cached.slice(0,30), near: [] };
  ...
  try{
    ...
    const [perfectRes, nearRes, soundLikeRes] = await Promise.all([
      fetch(`https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&md=fs&max=140`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=fs&max=140`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?sl=${encodeURIComponent(w)}&md=fs&max=100`, { signal: rhymeAbort.signal })
    ]);
...
function renderRhymeScroller(perfect, near){
  ...
  const pushSection = (label, words) => {
    ...
    parts.push(`<div class="rhymeSectionTag small">${escapeHtml(label)}</div>`);
    for(const col of chunkRhymeColumns(words)){
      parts.push(`
        <div class="rhymeStackCol">
          ${col.map(w=>`<button type="button" class="rhymeChip" data-rhyme="${escapeHtml(w)}">${escapeHtml(w)}</button>`).join("")}
        </div>
      `);
    }
  };
...
function renderRhymes(groups){
  ...
  const perfect = Array.isArray(groups) ? groups.slice(0,30) : (Array.isArray(groups?.perfect) ? groups.perfect : []);
