...
function applyNotesTypingColor(color, persist=false){
  try{
    ...
    // Important: do NOT set editor.style.color here for non-empty notes.
    // That can recolor inherited/plain text across the whole note.
    // We only persist/sync the current typing color picker value and reseed
    // empty editable lines so typing after deleting everything stays in color.
    if(ed){
    ...
    syncEmptyNotesEditableColor(getCurrentNotesEditable?.(), c);
...
function saveNotesSelection(){
  try{
    ...
    activeNotesEditable = getChecklistTextFromNode(range.startContainer)
      || (range.startContainer?.nodeType === Node.ELEMENT_NODE ? range.startContainer.closest?.('.notes-plain-line') : range.startContainer?.parentElement?.closest?.('.notes-plain-line'))
      || ed;
  }catch(_e){}
}

function getCurrentNotesEditable(){
  try{
    const ed = els.notesEditor;
    if(activeNotesEditable && ed?.contains(activeNotesEditable)) return activeNotesEditable;
    const sel = window.getSelection?.();
    const node = sel?.rangeCount ? sel.getRangeAt(0).startContainer : null;
    const fromNode = getChecklistTextFromNode(node) || (node?.nodeType === Node.ELEMENT_NODE ? node.closest?.('.notes-plain-line') : node?.parentElement?.closest?.('.notes-plain-line'));
    activeNotesEditable = fromNode || ed || null;
    return activeNotesEditable;
  }catch(_e){
    return els.notesEditor || null;
  }
}
...
function restoreNotesSelection(){
  try{
    ...
    if(lastNotesRange &&
       ed.contains(lastNotesRange.startContainer) &&
       ed.contains(lastNotesRange.endContainer)){
      sel.removeAllRanges();
...
function primeNotesTypingColor(color){
  try{
    ...
    const host = getCurrentNotesEditable() || ed;
    host.focus?.();
    restoreNotesSelection();
...
function openNotes(){
  ...
  activeNotesEditable = els.notesEditor;
  setDockHidden(true);
  updateRhymes("");
  requestAnimationFrame(()=>{
    ...
    primeNotesTypingColor(getNotesTypingColor());
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
    setTimeout(()=>{
      saveNotesSelection();
...
function getChecklistRowFromNode(node){
  try{
    if(!node) return null;
    const base = node.nodeType === Node.ELEMENT_NODE ? node : node.parentElement;
    return base?.closest?.('.notes-checklist-row') || null;
  }catch(_e){
    return null;
  }
}

function getChecklistTextFromNode(node){
  try{
    if(!node) return null;
    const base = node.nodeType === Node.ELEMENT_NODE ? node : node.parentElement;
    return base?.closest?.('.notes-checklist-text') || null;
  }catch(_e){
    return null;
  }
}

function isNotesNodeEmpty(node){
  try{
    if(!node) return true;
    const text = String(node.textContent || '').replace(/​/g, '').replace(/ /g, ' ').trim();
    if(text) return false;
    const meaningful = node.querySelector?.('img,hr,input,table,video,audio,iframe');
    return !meaningful;
  }catch(_e){
    return true;
  }
...
function seedNotesEditableColor(el){
  try{
    if(!el) return;
    const color = String(getNotesTypingColor() || '#111111');
    if(isNotesNodeEmpty(el)){
      el.style.color = color;
    }
  }catch(_e){}
}

function syncEmptyNotesEditableColor(el, color=null){
  try{
    if(!el) return;
    const c = String(color || getNotesTypingColor() || '#111111');
    if(el === els.notesEditor) return;
    if(isNotesNodeEmpty(el)){
      el.style.color = c;
      if(el.innerHTML === '' || el.innerHTML == null) el.innerHTML = '<br>';
    }
  }catch(_e){}
}

function splitChecklistTextAtCaret(textEl){
  try{
    const sel = window.getSelection?.();
    if(!textEl || !sel || !sel.rangeCount) return { beforeHtml: textEl?.innerHTML || '<br>', afterHtml: '' };
    const range = sel.getRangeAt(0);
    if(!textEl.contains(range.startContainer) || !textEl.contains(range.endContainer)){
      return { beforeHtml: textEl.innerHTML || '<br>', afterHtml: '' };
    }
    ...
    return {
      beforeHtml: wrap(beforeRange.cloneContents()) || '',
      afterHtml: wrap(afterRange.cloneContents()) || ''
    };
  }catch(_e){
    return { beforeHtml: textEl?.innerHTML || '<br>', afterHtml: '' };
  }
...
function createNotesChecklistRow(checked=false){
  const row = document.createElement('div');
  row.className = 'notes-checklist-row';
  row.setAttribute('data-checked', checked ? '1' : '0');
  row.setAttribute('contenteditable', 'false');

  const box = document.createElement('button');
  box.type = 'button';
  box.className = 'notes-checklist-box';
  box.setAttribute('contenteditable', 'false');
  box.setAttribute('tabindex', '-1');
  box.setAttribute('aria-label', checked ? 'Checked' : 'Unchecked');
...
function createNotesPlainLine(){
  const line = document.createElement('div');
  line.className = 'notes-plain-line';
  line.setAttribute('contenteditable', 'true');
  line.setAttribute('spellcheck', 'true');
  line.innerHTML = '<br>';
  seedNotesEditableColor(line);
  return line;
}

function setChecklistRowChecked(row, checked){
  try{
    if(!row) return;
    row.setAttribute('data-checked', checked ? '1' : '0');
    const box = row.querySelector('.notes-checklist-box');
    if(box){
      box.setAttribute('aria-label', checked ? 'Checked' : 'Unchecked');
      box.setAttribute('aria-pressed', checked ? 'true' : 'false');
    }
...
function focusChecklistTextStart(textEl){
  if(!textEl) return;
  activeNotesEditable = textEl;
  textEl.focus();
  if(isNotesNodeEmpty(textEl)){
    textEl.innerHTML = '<br>';
    seedNotesEditableColor(textEl);
  }
  placeCaretAtStart(textEl);
  saveNotesSelection();
}

function focusChecklistTextEnd(textEl){
  if(!textEl) return;
  activeNotesEditable = textEl;
  textEl.focus();
...
function insertChecklistRowAtSelection(){
  try{
    const ed = els.notesEditor;
    if(!ed) return false;
    ed.focus();
    ...
    let range = getNotesSelectionRange();
    if(!range){
      if(!normalizeNotesHtml(ed.innerHTML)) ed.innerHTML = '';
      const row = createNotesChecklistRow(false);
      ed.appendChild(row);
      focusChecklistTextStart(row.querySelector('.notes-checklist-text'));
      saveNotesFromEditor();
      scheduleNotesRhymeRefresh();
      return true;
    }

    let row = getChecklistRowFromNode(range.startContainer);
    if(row){
      focusChecklistTextEnd(row.querySelector('.notes-checklist-text'));
      return true;
    }

    const newRow = createNotesChecklistRow(false);
    let block = range.startContainer.nodeType === Node.ELEMENT_NODE ? range.startContainer : range.startContainer.parentElement;
    block = block?.closest?.('.notes-plain-line, div, p, li');

    if(block && block !== ed && ed.contains(block) && !block.classList?.contains('notes-checklist-text')){
      const html = String(block.innerHTML || '').trim();
      const hasMeaningful = normalizeNotesHtml(html || '') || String(block.textContent || '').trim();
      if(hasMeaningful){
        newRow.querySelector('.notes-checklist-text').innerHTML = html || '<br>';
      }
      block.replaceWith(newRow);
    }else{
      if(!normalizeNotesHtml(ed.innerHTML)) ed.innerHTML = '';
      range.collapse(false);
      range.insertNode(newRow);
    }

    focusChecklistTextStart(newRow.querySelector('.notes-checklist-text'));
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
    return true;
  }catch(_e){
...
function handleChecklistEnter(textEl){
  try{
    if(!textEl) return false;
    const row = textEl.closest('.notes-checklist-row');
    if(!row) return false;

    if(isNotesNodeEmpty(textEl)){
      const plain = createNotesPlainLine();
      row.replaceWith(plain);
      activeNotesEditable = plain;
      plain.focus?.();
      placeCaretAtStart(plain);
      saveNotesSelection();
    }else{
      const parts = splitChecklistTextAtCaret(textEl);
      const beforeHtml = normalizeNotesHtml(parts.beforeHtml || '') || '<br>';
      const afterHtml = normalizeNotesHtml(parts.afterHtml || '');

      textEl.innerHTML = beforeHtml;
      const newRow = createNotesChecklistRow(false);
      const newText = newRow.querySelector('.notes-checklist-text');
      if(afterHtml){
        newText.innerHTML = afterHtml;
      }
      row.insertAdjacentElement('afterend', newRow);
      focusChecklistTextStart(newText);
    }
...
function focusNotesEditableEndFromRowCandidate(node){
  try{
    if(!node) return false;
    const checklistText = node.matches?.('.notes-checklist-text')
      ? node
      : node.querySelector?.('.notes-checklist-text');
    if(checklistText){
      focusChecklistTextEnd(checklistText);
      return true;
    }
    const plain = node.matches?.('.notes-plain-line')
      ? node
      : node.querySelector?.('.notes-plain-line');
    if(plain){
      activeNotesEditable = plain;
      plain.focus?.();
...
function handleChecklistBackspace(textEl){
  try{
    if(!textEl || !isNotesNodeEmpty(textEl)) return false;
    const row = textEl.closest('.notes-checklist-row');
    if(!row) return false;
    const ed = els.notesEditor;
    ...
    let focused = false;
    if(prev) focused = focusNotesEditableEndFromRowCandidate(prev);
    if(!focused && next) focused = focusNotesEditableEndFromRowCandidate(next);

    if(!focused && ed){
      const plain = createNotesPlainLine();
      ed.appendChild(plain);
      activeNotesEditable = plain;
      plain.focus?.();
      placeCaretAtStart(plain);
      saveNotesSelection();
...
function getNotesEventElement(target){
  try{
...
function toggleChecklistRowFromTarget(target){
  try{
    const el = getNotesEventElement(target);
    const row = el?.closest?.('.notes-checklist-row');
    if(!row) return false;
    const next = row.getAttribute('data-checked') !== '1';
    setChecklistRowChecked(row, next);
    const text = row.querySelector('.notes-checklist-text');
    if(text){
      activeNotesEditable = text;
      focusChecklistTextEnd(text);
    }
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
    return true;
...
function wireNotesEditor(){
  ...
  ed.addEventListener("beforeinput", (e)=>{
    const textEl = getChecklistTextFromNode(e.target) || getChecklistTextFromNode(window.getSelection?.()?.anchorNode);
    const plainEl = (!textEl) ? ((getNotesEventElement(e.target)?.closest?.('.notes-plain-line')) || (window.getSelection?.()?.anchorNode ? (getNotesEventElement(window.getSelection()?.anchorNode)?.closest?.('.notes-plain-line')) : null)) : null;
    const activeEl = textEl || plainEl || getCurrentNotesEditable();
    if((e.inputType === "insertText" || e.inputType === "insertCompositionText") && activeEl && isNotesNodeEmpty(activeEl)){
      const typingColor = getNotesTypingColor();
      syncEmptyNotesEditableColor(activeEl, typingColor);
      primeNotesTypingColor(typingColor);
    }
    if(!textEl) return;
    if(e.inputType === "insertParagraph"){
      e.preventDefault();
      handleChecklistEnter(textEl);
      return;
    }
    if(e.inputType === "deleteContentBackward"){
      if(isNotesNodeEmpty(textEl)) {
        e.preventDefault();
        handleChecklistBackspace(textEl);
      }
    }
  });
  ...
  ed.addEventListener("keydown", (e)=>{
    const textEl = getChecklistTextFromNode(e.target) || getChecklistTextFromNode(window.getSelection?.()?.anchorNode);
    if(e.key === "Enter" && textEl){
      e.preventDefault();
      handleChecklistEnter(textEl);
      return;
    }
    if(e.key === "Backspace" && textEl && isNotesNodeEmpty(textEl)){
      e.preventDefault();
      handleChecklistBackspace(textEl);
      return;
    }
  ...
  let notesChecklistTapHandledAt = 0;
  let notesChecklistSuppressClickUntil = 0;
  const rowPointerHandler = (e)=>{
    const targetEl = getNotesEventElement(e.target);
    const isCheckboxTap = !!targetEl?.closest?.('.notes-checklist-box');
    const row = targetEl?.closest?.('.notes-checklist-row');
    const text = row?.querySelector?.('.notes-checklist-text');

    if(isCheckboxTap){
      const now = Date.now();
      if(now - notesChecklistTapHandledAt < 250){
        e.preventDefault();
        e.stopPropagation();
        return;
      }
      notesChecklistTapHandledAt = now;
      notesChecklistSuppressClickUntil = now + 450;
      e.preventDefault();
      e.stopPropagation();
      toggleChecklistRowFromTarget(targetEl);
      return;
    }

    if(row && text && !targetEl?.closest?.('.notes-checklist-text')){
      e.preventDefault();
      focusChecklistTextEnd(text);
    }
  };
  if(window.PointerEvent){
    ed.addEventListener("pointerdown", rowPointerHandler);
  }else{
    ed.addEventListener("touchstart", rowPointerHandler, { passive:false });
    ed.addEventListener("mousedown", rowPointerHandler);
  }
  ed.addEventListener("click", (e)=>{
    const targetEl = getNotesEventElement(e.target);
    if(targetEl?.closest?.('.notes-checklist-box')){
      if(Date.now() <= notesChecklistSuppressClickUntil){
        e.preventDefault();
        e.stopPropagation();
        return;
      }
    ...
    if(targetEl?.classList?.contains('notes-checklist-text') || targetEl?.classList?.contains('notes-plain-line')){
      activeNotesEditable = targetEl;
    }
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
  });
  ed.addEventListener("focusin", (e)=>{
    const t = getNotesEventElement(e.target) || e.target;
    if(t?.classList?.contains('notes-checklist-text') || t?.classList?.contains('notes-plain-line')){
      activeNotesEditable = t;
    }else if(t === ed){
      activeNotesEditable = ed;
    }
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
  });
  ed.addEventListener("input", ()=>{
    applyNotesTypingColor(getNotesTypingColor(), false);
    const activeEl = getCurrentNotesEditable();
    syncEmptyNotesEditableColor(activeEl, getNotesTypingColor());
    if(normalizeNotesHtml(ed.innerHTML)){
      ed.style.removeProperty("color");
    }
    saveNotesSelection();
  ...
  ed.addEventListener("focus", ()=>{
    activeNotesEditable = getCurrentNotesEditable() || ed;
    applyNotesTypingColor(getNotesTypingColor(), false);
    syncEmptyNotesEditableColor(activeNotesEditable, getNotesTypingColor());
    if(document.activeElement === ed){
      try{ primeNotesTypingColor(getNotesTypingColor()); }catch(_e){}
    }
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
  });
...
[
  ...
  el?.addEventListener("touchstart", ()=> saveNotesSelection(), { passive:true });
  el?.addEventListener("pointerdown", ()=> saveNotesSelection());
});
...
els.notesChecklistBtn?.addEventListener("click", ()=> insertChecklistRowAtSelection());
