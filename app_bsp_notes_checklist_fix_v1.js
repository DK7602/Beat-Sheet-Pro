...
function notesTypingSeedHtml(){
  const c = getNotesTypingColor();
  return `<span data-notes-typing-color="${escapeHtml(c)}" style="color:${escapeHtml(c)};">​</span>`;
}

function createNotesPlainLine(initialHtml=""){
  const line = document.createElement("div");
  line.className = "notesPlainLine";
  line.innerHTML = initialHtml || notesTypingSeedHtml();
  return line;
}
...
function createNotesCheckRow(textHtml="", checked=false){
  ...
  text.innerHTML = textHtml || notesTypingSeedHtml();
...
function wireNotesEditor(){
  ...
  const syncChecklistRow = (box)=>{
    const row = box?.closest?.('.notesCheckRow');
    if(!box || !row) return;
    row.classList.toggle('isChecked', !!box.checked);
    saveNotesSelection();
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
  };
