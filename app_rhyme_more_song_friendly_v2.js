...
const RHYME_CACHE_KEY = `${KEY_PREFIX}rhyme_cache_v3`;
...
function buildRhymeBuckets(seed, perfectData, nearData, soundLikeData){
  ...
  return {
    perfect: perfect.slice(0, 20).map(x=>x.word),
    near: near.slice(0, 20).map(x=>x.word)
  };
}
...
async function updateRhymes(seed){
  ...
  if(Array.isArray(cached) && cached.length){
    const fallbackBuckets = { perfect: cached.slice(0,20), near: [] };
...
function renderRhymes(groups){
  ...
  const perfect = Array.isArray(groups) ? groups.slice(0,20) : (Array.isArray(groups?.perfect) ? groups.perfect.slice(0,20) : []);
  const near = Array.isArray(groups?.near) ? groups.near.slice(0,20) : [];
